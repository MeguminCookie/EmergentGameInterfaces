    ...
    public class Joycon
    {
        ...
        private struct Rumble
        {
            ...
            public byte[] GetData()
            {
                ...
                l_f = Mathf.Clamp(l_f, 40.875885f, 626.286133f);
                amp = Mathf.Clamp(amp, 0.0f, 1.0f);
                h_f = Mathf.Clamp(h_f, 81.75177f, 1252.572266f);
                short hf = (short)((Mathf.Round(32f * Mathf.Log(h_f * 0.1f, 2)) - 0x60) * 4);
                ...
                ushort lf_amp = (ushort)(Mathf.Round(hf_amp) * .5);
        ...
        private float err;
        private Vector3 i_b, j_b, k_b, k_acc;
        ...
        public event Action<bool, Button> OnButtonPressed;
        public event Action<bool, Button> OnButtonReleased;
        ...
        public Joycon(IntPtr handle_, bool imu, bool localize, float alpha, bool left)
        {
            ...
            filterweight = alpha;
            isLeft = left;
            rotationFix = Quaternion.AngleAxis(90, Vector3.right);
        }
        ...
        /// <summary>
        /// Returns the acceleration vector of the controller in local space in G's or multiples of m/s^2
        /// When the controller is laying down flat it should return around Vector3(0, -1, 0);
        /// </summary>
        /// <returns></returns>
        ...
        {
            return GetOrientation() * GetAccelerationLocal();
        }

        public Vector3 GetAccelerationWorldWithoutGravity()
        {
            return GetAccelerationWorld() + Vector3.up;
        }
        ...
        public Quaternion GetOrientation()
        {
            ...
            if (v2 != Vector3.zero)
            {
                return rotationFix * Quaternion.LookRotation(v1, v2);
            }
            else
            {
        ...
        public void Update()
        {
            if (state > state_.NO_JOYCONS)
            {
                ...
                bool[] previousButtons = new bool[buttons.Length];
                ...
                while (reports.Count > 0)
                {
                    ...
                    DebugPrint(string.Format("Dequeue. Queue length: {0:d}. Packet ID: {1:X2}. Timestamp: {2:X2}. Lag to dequeue: {3:t}. Lag between packets (expect 15ms): {4:g}",
                        reports.Count, report_buf[0], report_buf[1], DateTime.Now.Subtract(rep.GetTime()), rep.GetTime().Subtract(ts_prev)), DebugType.THREADING);
                    ts_prev = rep.GetTime();
                }
                ...
                for (int i = 0; i < buttons.Length; ++i)
                {
                    if (previousButtons[i] != buttons[i])
                    {
                        ...
                        if (buttons[i])
                        {
                            OnButtonPressed(isLeft, (Button)i);
                        }
                        else
                        {
                            OnButtonReleased(isLeft, (Button)i);
                        }
                    }   
                }
        ...
        private Vector2 CenterSticks(ushort[] vals)
        {
            /*
                float[] s = { 0, 0 };
                for (uint i = 0; i < 2; ++i)
            ...
            float diffX = vals[0] - stick_cal[2];

            if (Math.Abs(diffX) >= deadzone)
            {
                if (diffX > 0) // if axis is above center
                {
                    stickValue.x = diffX / stick_cal[0];
                }
                else
                {
                    stickValue.x = diffX / stick_cal[4];
                }
            }

            float diffY = vals[1] - stick_cal[3];

            if (Math.Abs(diffY) >= deadzone)
            {
                if (diffY > 0) // if axis is above center
                {
                    stickValue.y = diffY / stick_cal[1];
                }
                else
                {
                    stickValue.y = diffY / stick_cal[5];
                }
            }
        ...
        private void dump_calibration_data()
        {
            ...
            for (int i = 0; i < 9; ++i)
            {
                if (buf_[i] != 0xff)
                {
                    Debug.Log("[Joycon] Using user stick calibration data.");
                    found = true;
                    break;
                }
            }
            if (!found)
            {
                Debug.Log("[Joycon] Using factory stick calibration data.");
            ...
            if (gyr_neutral[0] + gyr_neutral[1] + gyr_neutral[2] == -3 || Math.Abs(gyr_neutral[0]) > 100 || Math.Abs(gyr_neutral[1]) > 100 || Math.Abs(gyr_neutral[2]) > 100)
            {
                ...
                gyr_neutral[0] = (short)(buf_[3] | ((buf_[4] << 8) & 0xff00));
                gyr_neutral[1] = (short)(buf_[5] | ((buf_[6] << 8) & 0xff00));
                gyr_neutral[2] = (short)(buf_[7] | ((buf_[8] << 8) & 0xff00));
